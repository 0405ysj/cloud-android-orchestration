...
const (
	...
	stateMsgLockUploadDir   = "Waiting if another host is uploading same artifacts"
)
...
func uploadFiles(srv hoclient.HostOrchestratorService, names []string, statePrinter *statePrinter) (string, error) {
	...
	statePrinter.Print(stateMsgLockUploadDir)
	uploadCompleted, err := srv.LockUploadDir(uploadDir)
	statePrinter.PrintDone(stateMsgLockUploadDir, err)
	if err != nil {
